...
func setRawTerminal(fd int) (*termState, error) {
	...
	if _, _, errno := syscall.Syscall6(syscall.SYS_IOCTL, uintptr(fd), 0x5401 /* TCGETS */, uintptr(unsafe.Pointer(&old)), 0, 0, 0); errno != 0 {
		return nil, errno
	}
	...
	if _, _, errno := syscall.Syscall6(syscall.SYS_IOCTL, uintptr(fd), 0x5402 /* TCSETS */, uintptr(unsafe.Pointer(&raw)), 0, 0, 0); errno != 0 {
		return nil, errno
	}
	return state, nil
}
