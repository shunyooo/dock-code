...
enum LauncherScriptGenerator {
	...
	static func generate() {
		...
		if [ -n "$BELVE_SSH_HOST" ]; then
		    ...
		    TMUX_APPLY="tmux set -t $TMUX_SESSION status off; tmux set -t $TMUX_SESSION prefix None; tmux set -t $TMUX_SESSION mouse on; tmux set -t $TMUX_SESSION escape-time 0; tmux setw -t $TMUX_SESSION pane-border-status off; tmux set -s set-clipboard on; tmux set -as terminal-features ',xterm-256color:clipboard'; tmux bind-key -T copy-mode-vi MouseDragEnd1Pane send-keys -X copy-selection-and-cancel; tmux bind-key -T copy-mode MouseDragEnd1Pane send-keys -X copy-selection-and-cancel; tmux bind-key -T copy-mode-vi Enter send-keys -X copy-selection-and-cancel; tmux bind-key -T copy-mode Enter send-keys -X copy-selection-and-cancel"

		    if [ -n "$BELVE_DEVCONTAINER" ] && [ -n "$BELVE_REMOTE_PATH" ]; then
		        # DevContainer: use tmux inside the container for session persistence
		        /usr/bin/ssh $SSH_OPTS -t "$BELVE_SSH_HOST" "export TERM=xterm-256color; cd $BELVE_REMOTE_PATH && devcontainer up --workspace-folder . >/dev/null 2>&1 && devcontainer exec --workspace-folder . sh -c 'S=$TMUX_SESSION; if command -v tmux >/dev/null 2>&1; then tmux has-session -t \$S 2>/dev/null || tmux new-session -d -s \$S; tmux set -t \$S status off 2>/dev/null; tmux set -t \$S prefix None 2>/dev/null; tmux set -t \$S mouse on 2>/dev/null; tmux set -t \$S escape-time 0 2>/dev/null; tmux setw -t \$S pane-border-status off 2>/dev/null; tmux set -s set-clipboard on 2>/dev/null; tmux set -as terminal-features \",xterm-256color:clipboard\" 2>/dev/null; tmux bind-key -T copy-mode-vi MouseDragEnd1Pane send-keys -X copy-selection-and-cancel 2>/dev/null; tmux bind-key -T copy-mode MouseDragEnd1Pane send-keys -X copy-selection-and-cancel 2>/dev/null; tmux bind-key -T copy-mode-vi Enter send-keys -X copy-selection-and-cancel 2>/dev/null; tmux bind-key -T copy-mode Enter send-keys -X copy-selection-and-cancel 2>/dev/null; exec tmux attach -t \$S; else exec \$SHELL -l; fi' 2>/dev/null"
