...
### 4-1. ペイン操作
- PaneContainer 内のペイン間移動（上下左右）
- ペインの分割（水平/垂直）
- ペインの閉じる
- ペインのリサイズ（幅/高さ調整）
- ペインのズーム（最大化/元に戻す）
- tmux 互換のキーバインド設定（prefix + 矢印キー等）

### 4-2. プロジェクト切り替え
- 次/前のプロジェクトに切り替え
- プロジェクト番号指定で直接切り替え（prefix + 1, 2, 3...）
- プロジェクト一覧のクイックピッカー表示

### 4-3. フォーカス移動
- PaneContainer ↔ VS Code ワークベンチ間のフォーカス移動
- ProjectBar ↔ PaneContainer ↔ ワークベンチの循環移動
